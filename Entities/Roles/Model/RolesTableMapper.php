...
final class RolesTableMapper extends TableMapper
{
    ...
    const COLUMN_NAMES = ['id', 'role', 'created'];
    ...
    {
        parent::__construct(self::TABLE_NAME, implode(",", self::COLUMN_NAMES), 'id');
        $this->setRoles();
    }
    ...
    public function getIdSelectField(array $fieldAttributes, string $fieldLabel, ?int $selectedOption, ?string $fieldError)
    {
    ...
    public function getObjects(array $whereColumnsInfo = null, string $orderBy = 'id'): array 
    {
        $roles = [];

        if (null !== $records = $this->select("*", $whereColumnsInfo, $orderBy)) {
            foreach ($records as $record) {
