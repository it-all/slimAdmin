...
class SystemEventsModel extends MultiTableModel
{
    ...
    private function insert(string $title, int $eventType = 2, string $notes = null, int $adminId = null)
    {
        ...
        $q = new QueryBuilder("INSERT INTO ".self::TABLE_NAME." (event_type, title, notes, admin_id, ip_address, resource, request_method) VALUES($1, $2, $3, $4, $5, $6, $7)", $eventType, $title, $notes, $adminId, $_SERVER['REMOTE_ADDR'], $_SERVER['REQUEST_URI'], $_SERVER['REQUEST_METHOD']);
    ...
    public function select(array $filterColumnsInfo = null)
    {
        ...
        $fromClause = "FROM ".self::TABLE_NAME." se JOIN ".self::TYPES_TABLE_NAME." syet ON se.event_type = syet.id LEFT OUTER JOIN administrators ON se.admin_id = administrators.id";
    ...
    {
        $q = new QueryBuilder("SELECT COUNT(*) FROM ".self::TABLE_NAME." WHERE admin_id = $1", $adminId);
